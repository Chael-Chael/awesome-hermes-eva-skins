"""Build experimental quadrant-shaped braille-texture EVA skins."""
...
from image_to_rich_braille import (
    ...
    render_pixel_art_textured_quadrants,
    yaml_block,
)
...
def make_fragment(image_path: Path) -> str:
    ...
    return render_pixel_art_textured_quadrants(
        processed,
        background_rgb=background,
        background_tolerance=28.0,
        alpha_threshold=12,
...
def render_preview(fragments: list[tuple[str, str]]) -> Path:
    ...
    for name, rich in fragments:
        ...
        draw.text((12, 10), f"{name} textured", fill=(160, 185, 255))
        y = 34
        for row in rows:
            x = 16
            pos = 0
    ...
    for panel in panels:
        ...
        y += panel.height + 16
    out_path = OUT_DIR / "eva-textured-quadrant-preview.png"
...
def main() -> None:
    ...
    for name, image_name in ITEMS:
        ...
        fragment_path = OUT_DIR / f"{name}-textured-quadrant.yaml"
        fragment_path.write_text(yaml_block("banner_hero", rich) + "\n", encoding="utf-8", newline="\n")
        print(fragment_path)

    for spec in EVA_SPECS:
        skin_name = f"{spec.name}-textured"
        textured_spec = replace(
            spec,
            name=skin_name,
            description=spec.description.replace("colored braille hero", "colored textured quadrant hero"),
        )
        skin_path = SKINS_DIR / f"{skin_name}.yaml"
        skin_path.write_text(make_skin_yaml(textured_spec, fragment_by_name[spec.name]), encoding="utf-8", newline="\n")
        print(skin_path)
