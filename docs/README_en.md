...
- `cool-retro-frame-amber.hlsl` (more stylized; most terminal colors are remapped toward amber): stronger amber remapping, CRT glow, and retro atmosphere. Use it for showcases, recordings, screenshots, the NERV monochrome theme, or whenever you want the whole terminal to feel closer to an old amber display.
- `cool-retro-frame-readable.hlsl` (recommended for most people; preserves terminal command colors as much as possible): keeps more original hue separation and highlight contrast. Use it for long README sessions, code, logs, command output, and multi-color banner tuning.

> [!NOTE]
> **If you are not happy with the terminal look and want to fine-tune it**, give your agent the HLSL files plus [Hammster/windows-terminal-shaders](https://github.com/Hammster/windows-terminal-shaders) as a reference, then let it adjust the shader for you.
>
> **If you want to customize more of the Hermes theme yourself**, such as colors, status indicator icons, and copy, use [cocktailpeanut/hermes-mod](https://github.com/cocktailpeanut/hermes-mod) for visual editing. After generating a config, let your agent merge the relevant parts into the theme YAML in this repo. Do not use the full config generated by hermes-mod directly, because it currently generates only monochrome Hermes Agent startup titles and heroes, so it cannot reproduce the multi-color effect used here.
>
> If the Hermes window logo is clipped or misaligned, try widening the terminal window or reducing the terminal font size.
...
3. To make new tabs open with this profile by default, open `Settings` -> `Startup`, set `Default profile` to `Cool Retro Frame Amber` or `Cool Retro Frame Readable`, then save.

![Set the new Windows Terminal profile as default](../assets/set-default.png)

4. Start Hermes. If the installer did not find the `hermes` command, run this inside Hermes:
...
In the Windows Terminal GUI, open the target profile, find `Additional settings`, then click `Appearance`:

![Windows Terminal Appearance settings location](../assets/pw-setting-location.png)

After opening `Appearance`, use these screenshots as a reference for the PowerShell profile settings. Font face, font size, color scheme, cursor, opacity, padding, and scrollbar settings should match the JSON snippet.
