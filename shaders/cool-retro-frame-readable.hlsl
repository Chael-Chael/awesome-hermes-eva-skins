...
#define CRT_SCREEN_BRIGHTNESS 1.2f
...
#define PHOSPHOR_HOT_GLOW 0.08f
...
float Colorfulness(float3 color)
{
    ...
    return (hi - lo) / max(hi, 0.0001f);
}

float3 ConvertToAmber(float3 sourceColor)
{
...
{
    float3 amber = ConvertToAmber(sourceColor);
    float luma = Luma(sourceColor);
    float colorful = Colorfulness(sourceColor);
    float preserve = CRT_COLOR_PRESERVE * smoothstep(0.12f, 0.36f, colorful) * smoothstep(0.10f, 0.35f, luma);
    float3 readableSource = max(sourceColor, CRT_BLACK_FLOOR);
    return lerp(amber, readableSource, preserve);
}
